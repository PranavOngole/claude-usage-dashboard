...
if git diff --quiet -- data/subscription_usage.json data/claudeai_usage.json \
   && [ -z "$(git status --porcelain -- data/claudeai_usage.json)" ]; then
  echo "No changes to commit"
  exit 0
fi

git add data/subscription_usage.json data/claudeai_usage.json
